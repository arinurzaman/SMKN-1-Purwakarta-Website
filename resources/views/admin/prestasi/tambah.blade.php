<p class="text-right">
  ...
  <form action="{{ asset('admin/prestasi/tambah_proses') }}" method="post" enctype="multipart/form-data" accept-charset="utf-8">
  {{ csrf_field() }}
  <div class="row form-group">
    <label class="col-md-3">Judul prestasi</label>
    <div class="col-md-6">
      <input type="text" name="judul_prestasi" class="form-control form-control-lg" placeholder="Judul prestasi" required="required" value="{{ old('judul_prestasi') }}">
    </div>
  </div>
  
  <div class="row form-group">
    <label class="col-md-3">Icon prestasi</label>
    <div class="col-md-6">
      <input type="text" name="icon" class="form-control" placeholder="Icon prestasi" value="{{ old('icon') }}">
  ...
  <div class="row form-group">
   ...
   <div class="col-md-3">
    <select name="id_kategoriprestasi" class="form-control select2">
     <?php foreach($kategori as $kategori) { ?>
       <option value="<?php echo $kategori->id_kategoriprestasi ?>"><?php echo $kategori->nama_kprestasi ?></option>
     <?php } ?>
   </select>
  </div>
  </div>
  ...
  <div class="row form-group">
    ...
    <div class="col-md-9">
      <textarea name="isi" class="form-control konten" placeholder="Isi prestasi">{{ old('isi') }}</textarea>
    </div>
  </div>
