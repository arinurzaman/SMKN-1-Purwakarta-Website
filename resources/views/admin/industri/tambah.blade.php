<p class="text-right">
  ...
  <form action="{{ asset('admin/industri/tambah_proses') }}" method="post" enctype="multipart/form-data" accept-charset="utf-8">
  {{ csrf_field() }}
  <div class="row form-group">
    <label class="col-md-3">Judul industri</label>
    <div class="col-md-6">
      <input type="text" name="judul_industri" class="form-control form-control-lg" placeholder="Judul industri" required="required" value="{{ old('judul_industri') }}">
    </div>
  </div>
  
  <div class="row form-group">
    <label class="col-md-3">Icon industri</label>
    <div class="col-md-6">
      <input type="text" name="icon" class="form-control" placeholder="Icon industri" value="{{ old('icon') }}">
   ...
   <div class="col-md-3">
    <select name="id_kategoriindustri" class="form-control select2">
     <?php foreach($kategori as $kategori) { ?>
       <option value="<?php echo $kategori->id_kategoriindustri ?>"><?php echo $kategori->nama_kindustri ?></option>
     <?php } ?>
   </select>
  </div>
  </div>
  ...
  <div class="row form-group">
    ...
    <div class="col-md-9">
      <textarea name="isi" class="form-control konten" placeholder="Isi industri">{{ old('isi') }}</textarea>
    </div>
  </div>
