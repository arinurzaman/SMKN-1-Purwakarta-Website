<p class="text-right">
  ...
  <form action="{{ asset('admin/jurusan/tambah_proses') }}" method="post" enctype="multipart/form-data" accept-charset="utf-8">
  {{ csrf_field() }}
  <div class="row form-group">
    <label class="col-md-3">Judul jurusan</label>
    <div class="col-md-6">
      <input type="text" name="judul_jurusan" class="form-control form-control-lg" placeholder="Judul jurusan" required="required" value="{{ old('judul_jurusan') }}">
    </div>
  </div>
  
  <div class="row form-group">
    <label class="col-md-3">Icon jurusan</label>
    <div class="col-md-6">
      <input type="text" name="icon" class="form-control" placeholder="Icon jurusan" value="{{ old('icon') }}">
   ...
   <div class="col-md-3">
    <select name="id_kategorijurusan" class="form-control select2">
     <?php foreach($kategori as $kategori) { ?>
       <option value="<?php echo $kategori->id_kategorijurusan ?>"><?php echo $kategori->nama_kjurusan ?></option>
     <?php } ?>
   </select>
  </div>
  </div>
  ...
  <div class="row form-group">
    ...
    <div class="col-md-9">
      <textarea name="isi" class="form-control konten" placeholder="Isi jurusan">{{ old('isi') }}</textarea>
    </div>
  </div>
